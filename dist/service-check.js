...
export function mountServiceCheck({state,plan,places,onActivate}) {
 ...
 host.innerHTML=`<span class="source-label">BEFORE YOU LEAVE</span><h3>Check your usual route</h3><p>Official alerts are checked separately from the replay above. Route times remain estimates.</p><button class="outline-button" id="check-official">Check official service alerts ↗</button><details><summary>Try assistance demos</summary><p>Synthetic examples, not current assistance.</p><button class="outline-button" data-assistance="publicBus">Free public-bus boarding</button><button class="outline-button" data-assistance="shuttle">Free MRT shuttle</button></details><div id="service-result" role="status" aria-live="polite"></div>`;
 ...
 async function busBridgeFor(route) {
  const ids=route.stationIds||[],a=ids.indexOf('paya'),b=ids.indexOf('kallang');if(a<0||b<0)return '';
  const from=a<b?'paya':'kallang',to=a<b?'kallang':'paya';
  try {
   ...
   return `<div class="service-segment"><strong>Verified regular-bus bridge</strong><p>These are ordinary paid services from official LTA stop and route data, separate from any free boarding or MRT shuttle announced above.</p>${data.options.slice(0,3).map(option=>`<p><strong>Bus ${esc(option.service)} · ${eta(option.arrival)}</strong><br>Walk ${option.walkFromMetres} m to ${esc(option.fromStop.name)} (${option.fromStop.code}), ride ${option.busStops} stops, then walk ${option.walkToMetres} m from ${esc(option.toStop.name)} (${option.toStop.code}). ${option.load?`Load: ${esc(option.load)}.`:''}</p>`).join('')}</div>`;
  } catch {return '';}
 }
 async function display(data) {
  ...
  if(impacts.length){
   ...
   try {
    ...
    const busBridge=await busBridgeFor(plan.usual);
