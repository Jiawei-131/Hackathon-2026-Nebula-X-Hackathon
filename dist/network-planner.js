...
export function planJourney({origin='tampines',destination='raffles',departure='07:40',deadline='08:45',scenario='disruption',comfort=false,threshold=10,priority="balanced",walkBreak=0,originLocation=null,blockedEdges=[]}={}){
 ...
 const routes=paths.slice(0,3).map(({id,path})=>{
  ...
  groups.forEach((group,i)=>{const first=group.edges[0],last=group.edges.at(-1);if(i)steps.push({title:`Change at ${places[first.from].name}`,detail:`Follow signs for ${lines[group.line]}. Interchange walking and waiting are estimated.`,minutes:5,type:'transfer'});steps.push({title:`Take ${lines[group.line]} to ${places[last.to].name}`,detail:`Board at ${places[first.from].name} · alight at ${places[last.to].name} · ${group.edges.length} stop${group.edges.length===1?'':'s'}`,minutes:group.edges.reduce((n,e)=>n+e.minutes,0),type:'rail'});});
 ...
 const usual=routes[0];let best=[...routes].sort((a,b)=>a.score-b.score)[0];
 // Stay on the usual route only for trivial timing variance. Never stick with it just because the numbers
 // are close when it's the one taking the active-incident penalty and a clean alternative avoids it entirely —
 // that would recommend riding into the reported fault.
 const skipsIncident=usual.delay>0&&best.delay===0;
 if(!comfort&&priority!=='simple'&&!skipsIncident&&usual.buffer>=threshold&&usual.max<=best.max+5)best=usual;
